#!/bin/bash
# ID: 1.1.12_var_includes.sh 1.1.4 Ensure nodev option set on /tmp partition (Automated)

section=1_initial_setup
sub_section=1.1_filesystem_configuration
script_name=1.1.12_var_includes.sh
...
echo "Executing: mount -o remount,nodev /tmp"
mount -o remount,nodev /tmp || result="pending"

# Goal: Secure the /etc/cron.daily directory by ensuring proper ownership and permissions.
# Filename: 5.1.4_cron_daily.sh
...
if [[ -d "$dir" ]] && \
  ...
  echo "OK: $dir ownership and permissions configured (CIS 5.1.4)."
  exit 0
else

# Goal: Secure the /etc/cron.hourly directory by ensuring proper ownership and permissions.
# Filename: 5.1.3_cron_hourly.sh
...
if [[ -d "$dir" ]]; then
  # Backup permissions (store output for verification; no file modification)
...
if [[ -d "$dir" ]] && \
  ...
  echo "OK: $dir ownership and permissions configured (CIS 5.1.3)."
  exit 0
else

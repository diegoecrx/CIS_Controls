#!/usr/bin/env bash
# 3.5.3.3.6 Ensure ip6tables is enabled and running (CIS Oracle Linux 7)
# Actions:
#   - Ensure iptables-services package present
#   - Unmask ip6tables.service (if masked)
#   - Enable and start ip6tables.service (systemctl --now enable ip6tables)
# Verification:
#   - systemctl is-enabled ip6tables == enabled
#   - systemctl is-active  ip6tables == active
#   - "ip6tables -L" succeeds (sanity check)
#
...
verify_state() {
  ...
  # Sanity: ip6tables should respond
  ip6tables -L >/dev/null 2>&1 || ok=0
  return $ok
}
...
main() {
  ...
  if verify_state; then
    echo "OK: ip6tables service is enabled and running (${CONTROL_ID})"
    exit 0
  else
    echo "FAIL: ip6tables service is not enabled and running (${CONTROL_ID})"
    exit 1
  fi
}

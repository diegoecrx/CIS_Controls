"#!/usr/bin/env bash
# 3.5.2.3 Ensure iptables-services not installed with nftables (CIS Oracle Linux 7)
# Actions:
...
main() {
  ...
  if [[ $FAIL -eq 0 ]]; then
    echo ""OK: iptables-services absent and legacy iptables units not active (${CONTROL_ID})""
    exit 0
  else
    echo ""FAIL: iptables-services still present or legacy iptables units active (${CONTROL_ID})""
    exit 1
  fi
}

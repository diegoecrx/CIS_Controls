...
# Goal: Collect events that modify the system's Mandatory Access Control (MAC) policy by monitoring SELinux directories.
# Filename: 4.1.6_mac_policy.sh
...
if [[ $ok -eq 1 ]]; then
  echo "OK: MAC policy audit rules configured (CIS 4.1.6)."
  exit 0
else
  echo "FAIL: Missing MAC policy audit rules." >&2
  exit 1
fi
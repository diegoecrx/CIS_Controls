...
# Goal: Collect kernel module loading and unloading events via audit rules.
# Filename: 4.1.16_modules.sh
...
rules_file="/etc/audit/rules.d/50-modules.rules"
...
if [[ $ok -eq 1 ]]; then
  echo "OK: Kernel module audit rules configured (CIS 4.1.16)."
  exit 0
else
  echo "FAIL: Missing kernel module audit rules." >&2
  exit 1
fi
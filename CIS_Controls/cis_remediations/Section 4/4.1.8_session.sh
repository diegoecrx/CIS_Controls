...
# Goal: Collect session initiation information by auditing utmp, wtmp and btmp files.
# Filename: 4.1.8_session.sh
...
if [[ $ok -eq 1 ]]; then
  echo "OK: Session audit rules configured (CIS 4.1.8)."
  exit 0
else
  echo "FAIL: Missing session audit rules." >&2
  exit 1
fi
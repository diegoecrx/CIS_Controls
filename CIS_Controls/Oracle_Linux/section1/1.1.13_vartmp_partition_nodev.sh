...
# 1.1.13_vartmp_partition_nodev.sh
# CIS Control - 1.1.13_vartmp_partition_nodev.sh
#
# FINAL DEFINITIVE VERSION - Proper error handling for all scenarios
###############################################################################

SCRIPT_NAME="1.1.13_vartmp_partition_nodev.sh"
...
# Ensure /tmp has nodev - handles existing /tmp mount
ensure_tmp_has_nodev() {
    log_message "INFO" "Ensuring /tmp has nodev option"

    # Check current mount
    if mount | grep " on /tmp " | grep -q "nodev"; then
        log_message "SUCCESS" "/tmp already has nodev option"
        return 0
    fi

    log_message "INFO" "/tmp does not have nodev, will configure it"
    ...
    if [ -z "$tmp_device" ]; then
        # /tmp not separately mounted - it's part of root
        log_message "INFO" "/tmp is not separately mounted (uses root filesystem)"
        log_message "INFO" "Will need reboot after adding tmpfs entry"
        ...
        log_message "WARNING" "Reboot required for /tmp tmpfs to take effect"
        ...
        if grep -q "$tmp_device.*[[:space:]]/tmp[[:space:]]" /etc/fstab; then
            # Found device entry in fstab
            if ! grep "$tmp_device.*[[:space:]]/tmp[[:space:]]" /etc/fstab | grep -q "nodev"; then
            ...
            # tmpfs entry
            if ! grep "tmpfs[[:space:]]*/tmp" /etc/fstab | grep -q "nodev"; then
                sed -i '/tmpfs[[:space:]]*\/tmp/ s/defaults/defaults,nodev,nosuid,noexec/' /etc/fstab
                log_message "SUCCESS" "Updated tmpfs /tmp entry in /etc/fstab"
            fi
        fi
        ...
        if mount -o remount,nodev,nosuid,noexec /tmp 2>/dev/null; then
            ...
            log_message "WARNING" "Could not remount /tmp immediately - reboot recommended"
            return 2
        fi
    fi
}
...
prompt_user() {
    echo ""
    echo "=========================================================================="
    echo "  CIS Benchmark 1.1.13 - /var/tmp nodev Requirement"
    echo "=========================================================================="
    echo ""
...
main_remediation() {
        ...
        if is_bind_mount; then
            ...
            if [ $reboot_required -eq 0 ]; then
                ...
                if mount | grep " on /var/tmp " | grep -q "nodev"; then
                    log_message "SUCCESS" "/var/tmp now has nodev (inherited from /tmp)"
                else
            ...
            if mount | grep " on /var/tmp " | grep -q "nodev"; then
                log_message "SUCCESS" "/var/tmp already has nodev"
            else
                backup_file "/etc/fstab"

                # Add nodev to fstab
                sed -i '/[[:space:]]\/var\/tmp[[:space:]]/ s/defaults/defaults,nodev/' /etc/fstab

                if mount -o remount,nodev /var/tmp 2>/dev/null; then
                    log_message "SUCCESS" "Remounted /var/tmp with nodev"
                else
    ...
    if [ $reboot_required -eq 1 ]; then
        ...
        if mount | grep " on /var/tmp " | grep -q "nodev"; then
            echo "? SUCCESS: /var/tmp has nodev option - CIS 1.1.13 COMPLIANT"
            log_message "SUCCESS" "CIS 1.1.13 COMPLIANT - /var/tmp has nodev"
        else
            echo "??  WARNING: nodev not yet active"
            log_message "WARNING" "nodev configuration incomplete"
        fi
    fi

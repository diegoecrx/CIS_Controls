...
SCRIPT_NAME="1.1.20_nodev_removable_partitions.sh"
...
create_udev_rule() {
    cat > /etc/udev/rules.d/99-removable-nodev.rules << 'EOF'
# CIS 1.1.20 - Removable media nodev
...
EOF
    ...
    log_message "SUCCESS" "Created udev rules for nodev enforcement"
}
...
main_remediation() {
    ...
    echo ""
    echo "========================================"
    echo "CIS 1.1.20 - nodev on Removable Media"
    echo "========================================"
    echo ""
    ...
    if [ $device_count -eq 0 ]; then
        ...
        for device in $removable_devices; do
            echo "Device: $device"
            if mount | grep "^$device " | grep -q "nodev"; then
                echo "  Status: COMPLIANT (has nodev)"
                log_message "SUCCESS" "$device has nodev"
            else
                echo "  Status: NON-COMPLIANT (missing nodev)"
                log_message "WARNING" "$device missing nodev"
            fi
        done
        echo ""
    fi
    ...
    echo "Future removable media will have nodev option"
    echo ""

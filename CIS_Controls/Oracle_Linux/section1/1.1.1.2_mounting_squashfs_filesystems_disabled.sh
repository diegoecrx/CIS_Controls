...
# 1.1.1.2_mounting_squashfs_filesystems_disabled.sh
# CIS Control - 1.1.1.2_mounting_squashfs_filesystems_disabled.sh
#
# This script implements proper CIS controls with comprehensive error handling
###############################################################################

SCRIPT_NAME="1.1.1.2_mounting_squashfs_filesystems_disabled.sh"
...
main_remediation() {
    ...
    # Disable squashfs filesystem mounting
    local modprobe_conf="/etc/modprobe.d/squashfs.conf"
    
    # Check if squashfs module is currently loaded
    if lsmod | grep -q "^squashfs"; then
        log_message "WARNING" "squashfs module is currently loaded"
    fi
    
    # Create or update modprobe configuration to disable squashfs
    if [ ! -f "$modprobe_conf" ]; then
        log_message "INFO" "Creating $modprobe_conf"
        echo "install squashfs /bin/true" > "$modprobe_conf"
        log_message "SUCCESS" "Created $modprobe_conf with squashfs disabled"
    else
        # Backup existing file
        backup_file "$modprobe_conf"
        
        # Check if squashfs is already disabled
        if grep -q "^install squashfs /bin/true" "$modprobe_conf"; then
            log_message "INFO" "squashfs is already disabled in $modprobe_conf"
        else
            # Remove any existing squashfs entries
            sed -i '/^install squashfs/d' "$modprobe_conf" 2>/dev/null || true
            
            # Add the disable directive
            echo "install squashfs /bin/true" >> "$modprobe_conf"
            log_message "SUCCESS" "Updated $modprobe_conf to disable squashfs"
        fi
    fi
    ...
    if lsmod | grep -q "^squashfs"; then
        if rmmod squashfs 2>/dev/null; then
            log_message "SUCCESS" "Unloaded squashfs kernel module"
        else
            log_message "WARNING" "Could not unload squashfs module (may be in use)"
        fi
    fi

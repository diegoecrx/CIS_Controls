<#
.SYNOPSIS
    CIS Control 17.5.5 - 17.5.5 (L1) Ensure 'Audit Other Logon/Logoff Events' is set to 'Success and Failure'
...
Write-Host "CIS Control 17.5.5 - Audit Policy Configuration" -ForegroundColor Cyan
Write-Host "========================================" -ForegroundColor Cyan
Write-Host ""
Write-Host "This script will configure the following audit policy:" -ForegroundColor Yellow
Write-Host "  Subcategory: Other Logon/Logoff Events" -ForegroundColor White
Write-Host "  Setting: Success and Failure" -ForegroundColor White
Write-Host ""
Write-Host "This will enable auditing of other logon/logoff events." -ForegroundColor Yellow
Write-Host ""
...
try {
    ...
    $result = auditpol /set /subcategory:"Other Logon/Logoff Events" /success:enable /failure:enable 2>&1
    ...
    if ($LASTEXITCODE -eq 0) {
        ...
        $verify = auditpol /get /subcategory:"Other Logon/Logoff Events" 2>&1
        Write-Host $verify
        
        Write-Host ""
        Write-Host "CIS Control 17.5.5 applied successfully!" -ForegroundColor Green
    } else {
    ...
    Write-Host "Error applying CIS Control 17.5.5: $_" -ForegroundColor Red
    exit 1
}

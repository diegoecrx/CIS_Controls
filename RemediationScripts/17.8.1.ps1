<#
.SYNOPSIS
    CIS Control 17.8.1 - 17.8.1 (L1) Ensure 'Audit Sensitive Privilege Use' is set to include 'Success and Failure'
...
Write-Host "CIS Control 17.8.1 - Audit Policy Configuration" -ForegroundColor Cyan
Write-Host "========================================" -ForegroundColor Cyan
Write-Host ""
Write-Host "This script will configure the following audit policy:" -ForegroundColor Yellow
Write-Host "  Subcategory: Sensitive Privilege Use" -ForegroundColor White
Write-Host "  Setting: Success and Failure" -ForegroundColor White
Write-Host ""
Write-Host "This will enable auditing of Sensitive Privilege Use events." -ForegroundColor Yellow
Write-Host ""
...
try {
    ...
    $result = auditpol /set /subcategory:"Sensitive Privilege Use" /success:enable /failure:enable 2>&1
    ...
    if ($LASTEXITCODE -eq 0) {
        ...
        $verify = auditpol /get /subcategory:"Sensitive Privilege Use" 2>&1
        Write-Host $verify
        
        Write-Host ""
        Write-Host "CIS Control 17.8.1 applied successfully!" -ForegroundColor Green
    } else {
    ...
    Write-Host "Error applying CIS Control 17.8.1: $_" -ForegroundColor Red
    exit 1
}

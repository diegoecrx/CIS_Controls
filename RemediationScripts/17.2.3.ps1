<#
.SYNOPSIS
    CIS Control 17.2.3 - 17.2.3 (L1) Ensure 'Audit User Account Management' is set to 'Success and Failure'
...
Write-Host "CIS Control 17.2.3 - Audit Policy Configuration" -ForegroundColor Cyan
Write-Host "========================================" -ForegroundColor Cyan
Write-Host ""
Write-Host "This script will configure the following audit policy:" -ForegroundColor Yellow
Write-Host "  Subcategory: User Account Management" -ForegroundColor White
Write-Host "  Setting: Success and Failure" -ForegroundColor White
Write-Host ""
Write-Host "This will enable auditing of user account management events." -ForegroundColor Yellow
Write-Host ""
...
try {
    ...
    $result = auditpol /set /subcategory:"User Account Management" /success:enable /failure:enable 2>&1
    ...
    if ($LASTEXITCODE -eq 0) {
        ...
        $verify = auditpol /get /subcategory:"User Account Management" 2>&1
        Write-Host $verify
        
        Write-Host ""
        Write-Host "CIS Control 17.2.3 applied successfully!" -ForegroundColor Green
    } else {

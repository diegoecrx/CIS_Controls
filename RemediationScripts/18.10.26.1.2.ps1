<#
.SYNOPSIS
    CIS Control 18.10.26.1.2 - 18.10.26.1.2 (L1) Ensure 'Application: Specify the maximum log file size (KB)' i
...
Write-Host "Applying CIS Control 18.10.26.1.2..." -ForegroundColor Cyan
...
try {
    ...
    if ($currentValue.$valueName -eq $value) {
        Write-Host "Successfully applied CIS Control 18.10.26.1.2" -ForegroundColor Green
    } else {
        Write-Host "Warning: Verification failed" -ForegroundColor Yellow
    }
    
} catch {
    Write-Host "Error applying CIS Control 18.10.26.1.2: $_" -ForegroundColor Red
    exit 1
}

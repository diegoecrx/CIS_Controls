...
$settings = $settingsDb.Settings | Where-Object { $_.Control -match '^18\.9' }

$results = @()
Write-Host ""
Write-Host "=== Section 18.9 - System ===" -ForegroundColor Cyan
...
Write-Host ""
Write-Host "Summary - 18.9: Applied $(($results | Where-Object {$_.Status -eq 'Success'}).Count), Failed $(($results | Where-Object {$_.Status -ne 'Success'}).Count)" -ForegroundColor Yellow

if ($Report) {
    $results | Export-Csv -Path "$PSScriptRoot\section18_18.9_report.csv" -NoTypeInformation
}

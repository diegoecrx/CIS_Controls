...
$settings = $settingsDb.Settings | Where-Object { $_.Control -match '^18\.7' }

$results = @()
Write-Host ""
Write-Host "=== Section 18.7 - Printers ===" -ForegroundColor Cyan
...
Write-Host ""
Write-Host "Summary - 18.7: Applied $(($results | Where-Object {$_.Status -eq 'Success'}).Count), Failed $(($results | Where-Object {$_.Status -ne 'Success'}).Count)" -ForegroundColor Yellow

if ($Report) {
    $results | Export-Csv -Path "$PSScriptRoot\section18_18.7_report.csv" -NoTypeInformation
}

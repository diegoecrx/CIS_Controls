...
$settings = $settingsDb.Settings | Where-Object { $_.Control -match '^18\.4' }

$results = @()
Write-Host ""
Write-Host "=== Section 18.4 - MS Security Guide ===" -ForegroundColor Cyan
...
Write-Host ""
Write-Host "Summary - 18.4: Applied $(($results | Where-Object {$_.Status -eq 'Success'}).Count), Failed $(($results | Where-Object {$_.Status -ne 'Success'}).Count)" -ForegroundColor Yellow

if ($Report) {
    $results | Export-Csv -Path "$PSScriptRoot\section18_18.4_report.csv" -NoTypeInformation
}
